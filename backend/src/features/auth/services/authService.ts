...
export class AuthService {
  ...
  async login(loginData: LoginRequest): Promise<ServiceResponse<LoginResponse>> {
    try {
      ...
      // Usar transacción para actualizar el último inicio de sesión y registrar el acceso
  ...
  async verifyUserActive(userId: number): Promise<ServiceResponse<boolean>> {
    try {
      ...
      if (isNaN(id)) {
        throw AppError.badRequest('ID de usuario inválido', ERROR_CODES.VALIDATION_ERROR);
      }
